...
```
Step 1 · 拆解 Deconstruct ── 5 Whys 挖真实动机 + 字段拆成 Stone/Opinion
Step 2 · 重建 Rebuild    ── 第一性原理定目标 + 3 个成功判据（含 1 个主判据）
Step 3 · 发散 Diverge    ── 横纵双轴分析 + 心理机制 → 6-10 个创意方向
Step 4 · 对抗 Adversarial ── Steelman + 13 攻击角度 → 找根本缺陷
Step 5 · 成型 Finalize   ── 七节方案（Big Idea/目标/体验/传播/执行/风险/评估）+ 三重自检
```

- **拆解 Deconstruct**：用 5 Whys 把「表面需求」和「真实动机」分开（钱 / 权 / 关系 / 怕），再把需求拆成字段表，标出哪些是「假硬约束」可挑战。

  Use 5 Whys to separate "surface request" from "real motive" (money / power / relationship / fear), then break the request into a field table and mark which "hard constraints" are actually soft.

- **重建 Rebuild**：抛开「同类活动怎么做」，回到第一性原理问三个问题——本质上达成什么？谁来、为什么来、记住什么？成功怎么判（3 个可量化判据 + 1 个主判据）？

  Put aside "what peers do" and ask from first principles — what must it achieve? who comes, why, and what will they remember? how is success measured (3 quantifiable criteria + 1 primary)?

- **发散 Diverge**：先做横纵双轴（纵向追时间：品类/品牌历史、翻车案例；横向切截面：同类活动、真实口碑），再从受众情绪底层选心理机制（好奇缺口 / 社交货币 / 稀缺 / 损失厌恶 / 身份认同 / 从众 / 情绪峰值），生成 6-10 个创意方向。

  Analyze on two axes first (diachronic: category/brand history, past failures; synchronic: peer events, real word-of-mouth), then pick psychology levers (curiosity gap / social currency / scarcity / loss aversion / identity / conformity / peak emotion) and generate 6-10 creative directions.

- **对抗 Adversarial**：Steelman 先行（把创意说到最强版再攻击），13 个攻击角度逐条过，收敛出「一戳就塌」的根本缺陷，每条高严重度缺陷给可执行修法。
...
- **成型 Finalize**：按七节骨架输出完整方案（一句话 Big Idea → 目标判据 → 体验设计 → 传播设计 → 执行设计 → 风险预案 → 效果评估），过 QA / 合规 / 算术三重自检，并写回案例库形成长闭环。

  Output the full plan across 7 sections (Big Idea → goals & criteria → experience → communication → execution → risk → evaluation), pass QA / compliance / arithmetic checks, and feed results back into the case library for a closed loop.
...
1. **拆到底**：只信不可再拆的事实，行业惯例/预算/形式默认可挑战。
   **Deconstruct to the bottom** — trust only irreducible facts; convention, budget, format are challengeable by default.
...
**文件结构** — 一个主文件 + 7 个引用文件，按需加载，读到哪段停哪段：
