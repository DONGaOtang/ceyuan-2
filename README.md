...
策元是一个把「模糊需求」加工成「可落地活动方案」的 AI Skill，覆盖发布会、年会、路演、展览、品鉴会、私享会、团建、招商会、快闪、市集、直播带货、峰会论坛、颁奖典礼、音乐会、公益等几乎所有活动类型。
...
1. **活动是手段，不是目的（第一性原理）** — 同是「发布会」，动机是「募资给投资人看」还是「卖货给渠道」，方案是两种物种。策元先用 5 Whys 把真实动机挖出来，再决定形式。动机错了，后面全错。
2. **自己攻击自己（Red Team + Steelman）** — 每个创意先被「钢化」成最强版本，再用 13 个攻击角度（凭什么来、预算错配、前提不成立……）打这个最强版，逼出那个「一戳破整个结构就塌」的根本缺陷。
3. **人不点头，不往下走（Human-in-the-loop）** — 不做全自动黑箱。五步每一步结束都强制暂停，等你确认才进入下一步。宁可慢，不做错方向。
4. **轻量活动不套重流程（复杂度分级）** — 团建（L1）走精简流程，发布会（L2）走完整五步，峰会（L3）再加完整检索。不会给 30 分钟的团建套 300 万发布会的流程。
5. **核心自包含 + 能力按需增强** — 内置方法能独立跑通全流程；执行时还会按「能力关键词」检索你环境里已有的 Skill 做增强，检索不到就用内置兜底，绝不硬依赖。

<a id="zh-pipeline"></a>
## 五步流程

每一步产出都必须过一轮对抗式审查，且必须等你确认才进入下一步。

```
Step 1 · 拆解     ── 5 Whys 挖真实动机 + 字段拆成 Stone/Opinion
Step 2 · 重建     ── 第一性原理定目标 + 3 个成功判据（含 1 个主判据）
Step 3 · 发散     ── 横纵双轴分析 + 心理机制 → 6-10 个创意方向
Step 4 · 对抗     ── Steelman + 13 攻击角度 → 找根本缺陷
Step 5 · 成型     ── 七节方案（Big Idea/目标/体验/传播/执行/风险/评估）+ 三重自检
```

- **拆解**：用 5 Whys 把「表面需求」和「真实动机」分开（钱 / 权 / 关系 / 怕），再把需求拆成字段表，标出哪些是「假硬约束」可挑战。
- **重建**：抛开「同类活动怎么做」，回到第一性原理问三个问题——本质上达成什么？谁来、为什么来、记住什么？成功怎么判（3 个可量化判据 + 1 个主判据）？
- **发散**：先做横纵双轴（纵向追时间：品类/品牌历史、翻车案例；横向切截面：同类活动、真实口碑），再从受众情绪底层选心理机制（好奇缺口 / 社交货币 / 稀缺 / 损失厌恶 / 身份认同 / 从众 / 情绪峰值），生成 6-10 个创意方向。
- **对抗**：Steelman 先行（把创意说到最强版再攻击），13 个攻击角度逐条过，收敛出「一戳就塌」的根本缺陷，每条高严重度缺陷给可执行修法。
- **成型**：按七节骨架输出完整方案（一句话 Big Idea → 目标判据 → 体验设计 → 传播设计 → 执行设计 → 风险预案 → 效果评估），过 QA / 合规 / 算术三重自检，并写回案例库形成长闭环。
...
一个主文件 + 7 个引用文件，按需加载，读到哪段停哪段：

```
SKILL.md                     # 主流程：五步 + 五条铁律 + 复杂度分级 + 数据流图
references/
├── creative-inputs.md       # 5 Whys 动机拆解 + 横纵双轴 + 心理机制清单
├── event-types.md           # 活动类型要素清单（6 大类 + 案例锚点）
├── industries.md            # 14 行业要点（调性/雷区/预算结构/合规红线）
├── adversarial.md           # 对抗审查规则 + 13 攻击角度
├── proposal-schema.md       # 方案最终七节结构模板
├── search-paths.md          # 检索路径库（去哪搜、怎么验证）
...
2. **It attacks its own ideas (Red Team + Steelman)** — every idea is first steelmanned to its strongest form, then attacked from 13 angles (why would anyone come, budget misallocation, false premise…) to expose the one fatal flaw that collapses the whole structure.
3. **No auto-pilot (Human-in-the-loop)** — every step force-stops and waits for human confirmation. Slow is fine; wrong direction is not.
4. **Complexity-aware routing** — team-building (L1) uses a lean path, product launch (L2) the full pipeline, summit (L3) adds full research. A 30-minute team-building won't get a 3-million-yuan launch process.
5. **Self-contained core + on-demand enhancement** — built-in methods run the whole flow; it also scans your environment for skills matching a "capability keyword" to enhance itself, and gracefully falls back when none exists. No hard dependency.
...
```
Step 1 · Deconstruct  ── 5 Whys for real motive + field table (Stone/Opinion)
Step 2 · Rebuild      ── first-principles goal + 3 success criteria (1 primary)
Step 3 · Diverge      ── diachronic/synchronic analysis + psychology → 6-10 ideas
Step 4 · Adversarial  ── Steelman + 13 attack angles → find the fatal flaw
Step 5 · Finalize     ── 7-section plan (Big Idea/goal/experience/communication/execution/risk/evaluation) + triple self-check
```

- **Deconstruct**: use 5 Whys to separate "surface request" from "real motive" (money / power / relationship / fear), break the request into a field table, and mark which "hard constraints" are actually soft.
- **Rebuild**: put aside "what peers do" and ask from first principles — what must it achieve? who comes, why, and what will they remember? how is success measured (3 quantifiable criteria + 1 primary)?
- **Diverge**: analyze on two axes first (diachronic: category/brand history, past failures; synchronic: peer events, real word-of-mouth), then pick psychology levers (curiosity gap / social currency / scarcity / loss aversion / identity / conformity / peak emotion) and generate 6-10 creative directions.
- **Adversarial**: Steelman first (strengthen the idea before attacking it), run through 13 attack angles, converge on the one fatal flaw, and give an actionable fix for every high-severity defect.
- **Finalize**: output the full plan across 7 sections (Big Idea → goals & criteria → experience → communication → execution → risk → evaluation), pass QA / compliance / arithmetic checks, and feed results back into the case library for a closed loop.
...
One main file + 7 reference files, loaded on demand:

```
SKILL.md                     # main flow: 5 steps + 5 rules + complexity routing + data flow
references/
├── creative-inputs.md       # 5 Whys + diachronic/synchronic analysis + psychology levers
├── event-types.md           # event-type playbook (6 categories + case anchors)
├── industries.md            # 14 industry notes (tone/red flags/budget/compliance)
├── adversarial.md           # adversarial review rules + 13 attack angles
├── proposal-schema.md       # 7-section proposal template
├── search-paths.md          # search-path library (where & how to verify)
...
> **Note**: keep `SKILL.md` at the root and the 7 files under `references/` untouched — don't rename or restructure them.
