...
| Step 1 信息采集与横纵分析 | 先检测采集能力和来源访问状态，再拿正向案例、反向吐槽、平台玩法和竞品空白 | 关键市场信号、正向机会、反向警告、待验证项 |
...
| Step 6 交付系统 | 把方向变成能排产、能验收、能降级的执行系统 | 最小执行版本、关键路径、责任摘要、最大卡点 |
| Step 7 Red Team 对抗 | 先把方案说到最强，再攻击策略风险和交付矛盾 | 最高风险、修法、失败降级 |
| Step 8 成案 | 按用途写成干净方案，并把推导另存为思考过程 | 活动方案 + 思考过程 |
| Step 9 终审模拟 | 模拟老板、客户、赞助商、执行团队和用户打回 | 判词、必改项、最高风险 |
